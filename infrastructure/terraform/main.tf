# Terraform configuration for mayo-example
...
terraform {
  ...
  backend "azurerm" {
    ...
    key                  = "mayo-example.terraform.tfstate"
  }
}
...
resource "azurerm_cosmosdb_account" "main" {
  ...
  name                = "mayo-example-cosmos-${var.environment}"
...
resource "azurerm_container_app" "main" {
  name                         = "mayo-example-${var.environment}"
  ...
  template {
    container {
      name   = "mayo-example"
      image  = "meijeracr.azurecr.io/mayo-example:latest"
      cpu    = 0.5
      memory = "1Gi"
